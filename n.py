...
os.system('xdg-open https://chat.whatsapp.com/IaQClZxI8af0O1wFx2BwZX?s=cl&p=a&ilr=4&amv=3');time.sleep(1.5)
...
{W}─────────────────────────────────────────────{W}
{W}[{G}•{W}]{G} DEVOLPER {W}:{G} rudra
{W}[{G}•{W}]{G} FACEBOOK {W}:{G} Rudra Pandit
{W}[{G}•{W}]{G} GITHUB   {W}:{G} rudra05
...
def createfb_method_3():
    ...
    for i in range(num):
        try:
            sys.stdout.write(f"\r{W} {G}Live  • {len(oks)} {R}CP • {len(cps)}{W} ")
            sys.stdout.flush()
            ...
            if "c_user" in login_coki:
                ...
                if status == "live":
                    ...
                    if show_details == 'y':
                        ...
                        print(f"\r{G}RUDRA{W}-{G}[OK] {uid} | {pww}")
                    try:
                   ...
                   # print(f"\r{R}RUDRA{W}-{R}[CP] {uid}|{pww}")
                    cps.append(uid)
...
def createfb_method_2():
    ...
    for i in range(num):
        try:
            sys.stdout.write(f"\r{W}RUDRA{G} OK • {len(oks)}   {R}CP • {len(cps)}{W} ")
            sys.stdout.flush()
            ses = requests.Session()
            response = ses.get("https://x.facebook.com/reg")
            form = extractor(response.text)
...
def createfb_method_1():
    ...
    for _ in range(num):
        try:
            sys.stdout.write(f"\r{W}RUDRA{G} OK • {len(oks)}{W} ")
            sys.stdout.flush()
            ses = requests.Session()
            response = ses.get("https://x.facebook.com/reg")
            form = extractor(response.text)
